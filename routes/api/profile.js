...
//ruta za current user profile
// @route GET api/profile
// @desc Get current users profile
// @access Private
//passport da bi utvrdili da je private route
router.get(
  "/",
  ...
  (req, res) => {
    ...
    Profile.findOne({ user: req.user.id })
      ...
      .then(profile => {
        if (!profile) {
          errors.nopofile = "There is no profile for this user";
...
router.get("/all", (req, res) => {
  ...
  Profile.find()
    ...
    .then(profiles => {
      if (!profiles) {
        errors.nopofile = "There are no profiles";
        return res.status(404).json(errors);
      }
      res.json(profiles);
    })
...
// @route DELETE api/profile/expirience/:_id
  ...
  (req, res) => {
    Profile.findOne({ user: req.user.id })
      .then(profile => {
        ...
        //Splice out of array
        profile.expirience.slice(removeIndex, 1);
  ...
  (req, res) => {
    Profile.findOne({ user: req.user.id })
      .then(profile => {
        ...
        //Splice out of array
        profile.education.slice(removeIndex, 1);
